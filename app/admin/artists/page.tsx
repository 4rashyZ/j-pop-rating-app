...
export default function AdminArtistsPage() {
  ...
  const currentPage = Math.min(page, Math.max(1, Math.ceil(matchingArtists.length / artistsPerPage)));
  const filteredArtists = useMemo(() => matchingArtists.slice((currentPage - 1) * artistsPerPage, currentPage * artistsPerPage), [matchingArtists, currentPage]);
  ...
  return <main className="page admin-catalogue-page">
    ...
    <nav className="admin-subnav" aria-label="Catalogue sections"><Link className="active" href="/admin/artists">Artists</Link><Link href="/admin/songs">Songs</Link><Link href="/admin/reviews">Reviews</Link><Link href="/admin">Editor</Link></nav>
    ...
    <Pagination currentPage={currentPage} totalItems={matchingArtists.length} pageSize={artistsPerPage} itemLabel="artists" onPageChange={setPage} />
  </main>;
}
