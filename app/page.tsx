...
export default function Home() {
  ...
  return <div className="page-wrap">
    ...
    <section className="discover-section">
      <div className="section-heading"><div><span className="eyebrow">CURATED FOR YOU</span><h2>Artists to discover</h2></div><label className="search-box"><SearchIcon /><span className="sr-only">Search artists</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search artists" /></label></div>
      {isSupabaseConfigured && <div className="genre-row" aria-label="Filter by genre">{genres.map((item) => <button className={genre === item ? "genre-chip selected" : "genre-chip"} onClick={() => setGenre(item)} key={item}>{item}</button>)}</div>}
